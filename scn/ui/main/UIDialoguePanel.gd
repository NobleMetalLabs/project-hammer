...
func _ready():
	...
	StoryEventHandler.display_storyevent.connect(set_current_story_event)
...
func update_ui() -> void:
	...
	if not _current_story_event is StoryChoiceEvent:
		button_one.text = "1. Continue"
		button_one.visible = true
		button_one.disabled = false
		return
	else:
		var curr_choices : Array[StoryChoiceEventChoice] = _current_story_event.choices
		for choice_idx in range(len(curr_choices)):
			var choice : StoryChoiceEventChoice = curr_choices[choice_idx]
...
func button_pressed(button_index : int) -> void:
	...
	var next_event : StoryEvent = null
	if not _current_story_event is StoryChoiceEvent:
		next_event = _current_story_event.next_event
	else:
		next_event = _current_story_event.choices[button_index].resulting_event
	StoryEventHandler.handle_story_event_advance(next_event)
